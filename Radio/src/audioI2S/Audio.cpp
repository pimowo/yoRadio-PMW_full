...
*/
bool Audio::openai_speech(
  const String &api_key, const String &model, const String &input, const String &instructions, const String &voice, const String &response_format,
  const String &speed
) {
  ps_ptr<char> host;
  ...
  String post_body = "{"
                     ...
                     "\"input\": \"" + input_clean + "\"," + "\"instructions\": \"" + instructions_clean + "\"," + "\"voice\": \"" + voice + "\","
...
int Audio::read_ID3_Header(uint8_t *data, size_t len) {
  ...
  if (m_controlCounter == MP3_TAG) {  // Read the value
    ...
    if (startsWith(m_ID3Hdr.tag, "COMM")) {  // language code
      ...
      if (cd_len > 2) {
        info(
          *this, evt_info, "Comment: text encoding; %s, language %s, content_descriptor: %s", encodingTab[textEncodingByte], m_ID3Hdr.lang,
          content_descriptor.c_get()
        );
      }
  ...
  if (m_controlCounter == MP3_SYLT) {  // SYLT
    ...
    if (m_dataMode == AUDIO_LOCALFILE || (m_streamType == ST_WEBFILE)) {
      ...
      info(
        *this, evt_info, "Lyrics: text_encoding: %s, language: %s, size %i", encodingTab[m_ID3Hdr.SYLT.text_encoding], m_ID3Hdr.SYLT.lang, m_ID3Hdr.SYLT.size
      );
  ...
  if (m_controlCounter == MP3_ID3V22) {  // frames in V2.2, 3bytes identifier, 3bytes size descriptor
    ...
    if (tag.starts_with("PIC")) {  // image embedded in header
      ...
      if (m_dataMode == AUDIO_LOCALFILE) {
        ...
        m_ID3Hdr.SYLT.lang[3] = '\0';
        info(
          *this, evt_info, "Lyrics: text_encoding: %s, language: %s, size %i",
...
int Audio::read_M4A_Header(uint8_t *data, size_t len) {
  ...
  if (m_controlCounter == M4A_BEGIN) {  // init
    ...
    if (atom_name.equals("ftyp")) {
      if (atom_struct) {
        AUDIO_LOG_WARN(
          "atom %s @ %zu, size: %u, ends @ %zu", atom_name.c_get(), m_m4aHdr.headerSize, (uint32_t)atom_size.to_uint32(16),
          m_m4aHdr.headerSize + (size_t)atom_size.to_uint32(16)
        );
  ...
  if (m_controlCounter == M4A_CHK) { /* check  Tag */
    ...
    if (atom_name.equals("moov")) {
      ...
      if (atom_struct) {
        AUDIO_LOG_WARN(
          "atom %s @ %i, size: %i, ends @ %i", atom_name.c_get(), m_m4aHdr.headerSize, atom_size.to_uint32(16), m_m4aHdr.headerSize + atom_size.to_uint32(16)
        );
      }
    ...
    if (atom_name.equals("mdat")) {
      ...
      if (!m_m4aHdr.progressive) {
        ...
        if (atom_struct) {
          AUDIO_LOG_WARN(
            "atom %s @ %i, size: %i, ends @ %i", atom_name.c_get(), m_m4aHdr.mdat_startPos, m_m4aHdr.sizeof_mdat, m_m4aHdr.mdat_startPos + m_m4aHdr.sizeof_mdat
          );
        }
...
void IRAM_ATTR Audio::playChunk() {
  ...
  if (m_bitsPerSample == 8) {
    m_plCh.sampleSize = 2 * getChannels();  // 1 byte upsampled (8 -> 16 bit) 2 bytes per sample (int16_t) * channels
  }
  if (m_bitsPerSample == 16) {
    m_plCh.sampleSize = 2 * getChannels();  // 2 bytes per sample (int16_t) * channels
  }
  if (m_bitsPerSample == 24) {
    m_plCh.sampleSize = 4 * getChannels();  // 3 bytes + padding per sample (int32_t) * channels
  }
  if (m_bitsPerSample == 32) {
    m_plCh.sampleSize = 4 * getChannels();  // 4 bytes per sample (int32_t) * channels
  }
  m_plCh.err = ESP_OK;
  m_plCh.i = 0;
...
void Audio::processLocalFile() {
  ...
  if (!m_f_stream) {
    if (m_controlCounter != 100) {
      ...
      if (InBuff.bufferFilled() > m_prlf.maxFrameSize || (InBuff.bufferFilled() == m_audioFileSize)
          || m_f_allDataReceived) {  // at least one complete frame or the file is smaller
...
void Audio::processWebFile() {
  ...
  if (!m_f_stream) {
    if (m_controlCounter != 100) {
      ...
      if (InBuff.bufferFilled() > m_pwf.maxFrameSize || (InBuff.bufferFilled() == m_audioFileSize)
          || m_f_allDataReceived) {  // at least one complete frame or the file is smaller
        InBuff.bytesWasRead(readAudioHeader(InBuff.readSpace()));
      }
      if (m_controlCounter == 100) {
      ...
      if (ws >= m_pwsHLS.ID3BuffSize - m_pwsHLS.ID3ReadPtr) {
        ...
        while (true) {
          if (InBuff.writeSpace() >= m_pwsHLS.ID3BuffSize - (m_pwsHLS.ID3ReadPtr + ws)) {
            memcpy(
              InBuff.getWritePtr(), &m_pwsHLS.ID3Buff[ws + m_pwsHLS.ID3ReadPtr], m_pwsHLS.ID3BuffSize - (m_pwsHLS.ID3ReadPtr + ws)
            );  // write everything that fits into the buffer
...
uint32_t Audio::decodeContinue(int8_t res, uint8_t *data, int32_t bytesDecoded, int32_t *bytesLeft) {
  ...
  if (m_codec == CODEC_AAC) {
    if (res == AACDecoder::AAC_ID3_HDR) {
      uint32_t size = ((data[6 + bytesDecoded] & 0x7F) << 21) | ((data[7 + bytesDecoded] & 0x7F) << 14) | ((data[8 + bytesDecoded] & 0x7F) << 7)
                      | (data[9 + bytesDecoded] & 0x7F);
...
// —————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————
void Audio::computeLimit() {     // obliczane przy zmianie głośności lub balansu
  const float MAX_GAIN = 0.33f;  // limiter maksymalnej głośności (33% pełnej mocy)
  ...
  // zastosuj obliczoną głośność i balans z limiterem maksymalnej mocy
  m_limit_left = l * v * MAX_GAIN;
  m_limit_right = r * v * MAX_GAIN;
  // AUDIO_LOG_INFO("m_limit_left %f, m_limit_right %f ",m_limit_left, m_limit_right);
}
// —————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————
void Audio::Gain(int16_t *sample) {
  /* ważne: wszystkie te mnożenia muszą być liczbami całkowitymi ze znakiem, w przeciwnym razie wynik będzie nieprawidłowy */
