  ...
  if (xQueueReceive(nsQueue, &request, NS_QUEUE_TICKS))
  {
    ...
    case NRSSI:
      sprintf(
          ...
          (player.isRunning() && config.store.audioinfo) ? (int)(100 * player.inBufferFilled() / playerBufMax) : 0); /*rssi = 255;*/
      break;
    case SDPOS:
    ...
    case EQUALIZER:
      ...
      break;
    case BALANCE:
      sprintf(wsBuf, "{\"payload\":[{\"id\": \"balance\", \"value\": %d}]}", config.store.balance);
      break;
    case SDINIT:
      sprintf(wsBuf, "{\"sdinit\": %d}", SDC_CS != 255);
      break;
    case GETPLAYERMODE:
...
void handleNotFound(AsyncWebServerRequest *request)
{
  ...
  if (request->method() == HTTP_POST)
  {
    ...
    if (request->url() == "/upload")
    { // <--upload playlist.csv or wifi.csv
